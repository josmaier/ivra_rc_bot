...
{
    public class PublishCommandIVRA(DiscordSocketClient client, ApplicationDbContext db, RaceControlClient raceControl) : InteractionModuleBase<SocketInteractionContext>
    {
        [SlashCommand("publishIVRA", "Publish a penalty to the team, notice board and ATVO Server")]
        public async Task PublishIVRAAsync([Summary("message", "The message to publish")] string message, [Summary("team-channel", "Channel to notify team")] ITextChannel teamChannel, [Summary("tag-role", "Role to mention in team channel")] IRole roleToTag, [Summary("id", "Protest Id")] int protestId, [Summary("number", "What is your car number?"), MinValue(1)] int number, [Summary("timestamp", "iRacing timestamp (HH:MM:SS)")] string timestamp, [Summary("category", "The Incident category")] IncidentCategory category, [Summary("session_type", "The session in which the incident occured")] IRacingSessionTypes? sessionType = null)
        {
            await DeferAsync(ephemeral: false);
            SocketGuildUser member = (SocketGuildUser)Context.User;
            if (!Program.IVRA)
            {
                await RespondAsync("This command is not available in the current environment.", ephemeral: true); return;
            }

            if (Program.rcOnlyCommandRoleList == null)
            {
                await FollowupAsync("Why the fuck is the main program gone?"); return;
            }

            if (!HelperFunctions.RoleCheck.HasRoles(member, Program.rcOnlyCommandRoleList))
            {
                await FollowupAsync("You do not have the permissions required to run this command", ephemeral: false); return;
            }
            ...
            {
                await FollowupAsync("Invalid or missing NOTICE_BOARD_CHANNEL_ID in .env", ephemeral: false); return;
            }

            if (db.Protests == null)
            {
                await FollowupAsync("Why the fuck is there no database"); return;
            }

            Match match = Regex.Match(message, @"^(.*?)\s+for\b", RegexOptions.IgnoreCase);
            string penalty = match.Success ? match.Groups[1].Value.Trim() : string.Empty;
            if (penalty == string.Empty)
            {
                await FollowupAsync("Could not parse the penalty from your input"); return;
            }
            ...
            string teamMessage = $"{roleToTag.Mention} for Car {number} for Inc. {protestId}.\n\n" +
                                 "You have 90 minutes to serve the penalty. Please acknowledge you have seen this message with a reaction.";

            string incidentCategory = category.GetType().GetField(typeof(ChoiceDisplayAttribute).)

            string noticeBoardMessage = $"Inc. {protestId} - {category.GetChoiceDisplayName()} - {penalty} for Car {number}";


            Embed embed = new EmbedBuilder().WithTitle("Penalty Notice")
                .WithDescription(message).WithColor(Color.Orange)
                .WithCurrentTimestamp()
                .Build();

            Embed teamEmbed = new EmbedBuilder().WithTitle("Penalty Notice")
                .WithDescription(message).AddField("Your Protest ID", protestId)
    ...
    {
        [ChoiceDisplay("1.1 – Incorrect Car in session")] IncorrectCarInSession = 11,
        [ChoiceDisplay("1.2 – Incorrect TeamID/Number in session")] IncorrectTeamIDOrNumber = 12,
        [ChoiceDisplay("1.3 – Non-registered Driver")] NonRegisteredDriver = 13,
        [ChoiceDisplay("1.4 – Drive time infringement")] DriveTimeInfringement = 14,
        [ChoiceDisplay("2.1 – Qualifying infringement")] QualifyingInfringement = 21,
        [ChoiceDisplay("2.2 – Start infringement")] StartInfringement = 22,
        [ChoiceDisplay("2.3 – Pit Lane infringement")] PitLaneInfringement = 23,
        [ChoiceDisplay("2.4 – Pit Lane Safety Car infringement")] PitLaneSafetyCarInfringement = 24,
        [ChoiceDisplay("2.5 – Safety Car Procedure infringement")] SafetyCarProcedureInfringement = 25,
        [ChoiceDisplay("2.5.A – Safety Car Procedure infringement (A)")] SafetyCarProcedureInfringementA = 251,
        [ChoiceDisplay("2.6 – Restart infringement")] RestartInfringement = 26,
        [ChoiceDisplay("3.1 – Minor incident responsibility")] MinorIncident = 31,
        [ChoiceDisplay("3.2 – Moderate incident responsibility")] ModerateIncident = 32,
        [ChoiceDisplay("3.3 – Major incident responsibility")] MajorIncident = 33,
        [ChoiceDisplay("3.4 – Blocking")] Blocking = 34,
        [ChoiceDisplay("3.5 – Not respecting track limits")] TrackLimits = 35,
        [ChoiceDisplay("3.6 – Overtaking outside track limits")] OvertakeOutsideTrack = 36,
        [ChoiceDisplay("3.7 – Unsafe rejoin")] UnsafeRejoin = 37,
        [ChoiceDisplay("3.8 – Aggressive driving")] AggressiveDriving = 38,
        [ChoiceDisplay("4.1 – Unsportsmanlike conduct")] UnsportsmanlikeConduct = 41
    }
}
